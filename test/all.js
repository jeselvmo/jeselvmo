...
describe('digitUppercase', () => {
    ...
    it('digitUppercase(10000.09) = \'壹万元玖分\'', () => {
        expect(
            digitUppercase(10000.09)
        ).toBe('壹万元玖分')
    });

    it('digitUppercase(-1.0912) = \'欠壹元玖分\'', () => {
        expect(
            digitUppercase(-1.0912)
        ).toBe('欠壹元玖分')
    });
...
describe('randomNum', () => {
    ...
    it('randomNum(1, 100) in [1, 100]', () => {
        expect(
            randomNum(1, 100)
        ).toBeGreaterThan(1).toBeLessThan(100)
    });

    it('randomNum(-50, 50) in [-50, 50]', () => {
        expect(
            randomNum(-50, 50)
        ).toBeGreaterThan(-50).toBeLessThan(50)
    });
});
...
describe('deserialize', () => {
    ...
    it('deserialize(string) = obj', () => {
        var obj = {name: 'yangkk'};
        expect(
            objectEquals(deserialize(JSON.stringify(obj)), obj)
        ).toBe(true)
    });
...
describe('digitUppercase', () => {
    ...
    it('digitUppercase(123456.12) = \'壹拾贰万叁仟肆佰伍拾陆元壹角贰分\'', () => {
        expect(
            digitUppercase(123456.12)
        ).toBe('壹拾贰万叁仟肆佰伍拾陆元壹角贰分')
    });
...
describe('escape2Html/html2Escape', () => {
    ...
    it('escape2Html(html2Escape(\'<html></html>\')) = \'<html></html>\'', () => {
        expect(
            escape2Html(html2Escape('<html></html>'))
        ).toBe('<html></html>')
    });
...
describe('formatDate', () => {
    ...
    it('formatDate(new Date(\'2018-10-31 15:47:06\')) = \'2018-10-31 15:47:06\'', () => {
        expect(
            formatDate(new Date('2018-10-31 15:47:06'))
        ).toBe('2018-10-31 15:47:06')
    });

    it('formatDate(new Date(\'2018-10-31\')) = \'2018-10-31\'', () => {
        expect(
            formatDate(new Date('2018-10-31'), 'yyyy-mm-dd')
        ).toBe('2018-10-31')
    });
...
describe('formatPassTime', () => {
    ...
    it('formatPassTime(new Date(\'2017-10-31\')) = \'1年前\'', () => {
        expect(
            formatPassTime(new Date('2017-10-31'))
        ).toBe('1年前')
    });
...
describe('formatRemainTime', () => {
    ...
    it('formatRemainTime(new Date(\'2017-10-31\')) = \'730天 16小时 2分钟 17秒\'', () => {
        expect(
            formatRemainTime(new Date('2020-10-31'))
        ).toNotBe('730天 16小时 2分钟 17秒')
    });
...
describe('toDate', () => {
    ...
    it('isDate(toDate(Date.now())) = true', () => {
        expect(
            isDate(toDate(Date.now()))
        ).toBe(true)
    });

    it('isDate(toDate(\'2018-12-31\')) = true', () => {
        expect(
            isDate(toDate('2018-12-31'))
        ).toBe(true)
    });

    it('toDate(\'15\', {min: 0, max: 10}) = false', () => {
        expect(
            isDate(toDate(new Date()))
        ).toBe(true)
    });
