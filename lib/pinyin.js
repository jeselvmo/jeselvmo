...
var _pinyinDict = require('./data/pinyin-dict');

var _pinyinDict2 = _interopRequireDefault(_pinyinDict);

var _phrasesDict = require('./data/phrases-dict');

var _phrasesDict2 = _interopRequireDefault(_phrasesDict);
...
var Pinyin = function () {
	...
	_createClass(Pinyin, [{
		...
		value: function convert(hans, options) {
			...
			options = (0, _objectAssign2.default)({}, DEFAULT_OPTIONS, options);
		...
		value: function phrases_pinyin(phrases, options) {
			...
			var py = [];
			if (_phrasesDict2.default.hasOwnProperty(phrases)) {
				//! copy pinyin result.
				_phrasesDict2.default[phrases].forEach(function (item, idx) {
					py[idx] = [];
...
var pinyin = new Pinyin(buildPinyinCache(_pinyinDict2.default));
